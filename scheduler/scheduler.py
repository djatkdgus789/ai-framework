...
def _get_schedulable_node(v1_client,end_device):
    node_list = _get_ready_nodes(v1_client)
    if not node_list:
        return None
    available_nodes = list(set(node_list))
    # return node that have the lowest RTT (server-device)
    # get network status from API server
    # network_status = requests.get('http://localhost:4000/api/rtt/'+end_device)
    # logger.info(network_status.json())
    logger.info("Get network_status")
    
    whale02 = {'t_comm': 42 , 't_comp' : 3.44 , 'gpu' : False}
    whale03 = {'t_comm' : 0.23, 't_comp': 3.44 , 'gpu' : False}
    orca02 = {'t_comm' : 42, 't_comp': 4.9 , 'gpu' : True}
    orca04 = {'t_comm' : 0.23, 't_comp': 4.9 , 'gpu' : True}


    t_comm_c = {'whale02' : 42, 'whale03' : 0.32}
    t_comm_g = {'ocra02' : 32, 'orca04' : 0.23}
    t_comp_c = {'whale02' : 3.44, 'whale03' : 3.44 }
    t_comp_g = { 'ocra02' : 4.9, 'orca04' : 4.9}

    C = min(t_comm_c, key=t_comm_c.get)
    G = min(t_comm_g, key=t_comm_g.get)

    if t_comm_c.get(C) + t_comp_c.get(C) <= t_comm_g.get(G) + t_comp_g.get(G):
        t_min = t_comm_c.get(C) + t_comp_c.get(C)
        minNode = C
    else :
        t_min = t_comm_g.get(C) + t_comp_g.get(C)
        minNode = G

    logger.info('Scheduler select feasible node %s: ',str(minNode))
    logger.info('Minimum response time is  %s: ',str(t_min))
